...
impl CommitOpt {
    ...
    pub fn commit_author(&self, conf_path: &Path) -> Result<Author> {
        if !conf_path.exists() {
            eprintln!("{} does not exist. Initialize a new config with `ofvr conf init'", &conf_path);
            std::process::exit(1);
        }
    ...
    match args.command {
        ...
        Command::Commit(op) => {
            ...
            let mut ofvr = if op.ofvr_state_path().is_file() {
                ...
                OFVRState::empty(&op.ofvr_state_path(), &author)?
            };
            ofvr.commit(&op.from_file, &author, &op.commit_message)?;
